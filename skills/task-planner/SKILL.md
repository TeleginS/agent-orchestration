---
name: task-planner
description: "Plan a feature, fix, or refactoring as tracker issues with acceptance criteria, dependencies, and size estimates; return the planning stage to its caller."
---
...
sensible subtask looks like here. Also read [tracker conventions](../../conventions/issue-tracker.md) for how this
...
In real mode, turn a high-level request into **tracker issues** — the sole planning
artifact. In dry-run mode, use the local plan specified by `PIPELINE.md`. Keep the
decomposition, dependencies, and acceptance criteria together in the selected artifact
so the developer, reviewer, and QA use the same source.
...
- **Not enough context to decompose properly** → return the concrete unresolved
  questions before creating issues. If an overview already exists, identify it in the
  blocked report; do not present it as a completed plan or invent child issues over a gap.
...
`.agents/memory/task-planner/MEMORY.md` if present and only the topic notes
